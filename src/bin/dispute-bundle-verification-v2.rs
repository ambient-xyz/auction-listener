...
mod tests {
    ...
    #[test]
    fn dispute_bundle_verification_v2_maps_cli_kinds() {
        assert_eq!(
            dispute_kind_from_arg(DisputeKindArg::MissedVerification),
            BundleVerificationDisputeV2Kind::MissedVerification
        );
        assert_eq!(
            dispute_kind_from_arg(DisputeKindArg::PaidVerdictDispute),
            BundleVerificationDisputeV2Kind::PaidVerdictDispute
        );
    }

    #[test]
    fn dispute_bundle_verification_v2_accepts_optional_refund_recipient() {
        let bundle_escrow = Pubkey::new_unique().to_string();
        let refund_recipient = Pubkey::new_unique().to_string();

        let cli = Cli::try_parse_from([
            "dispute-bundle-verification-v2",
            "--dispute-payer-keypair",
            "payer.json",
            "--bundle-escrow",
            &bundle_escrow,
            "--kind",
            "paid-verdict-dispute",
            "--bond-refund-recipient",
            &refund_recipient,
        ])
        .unwrap();

        assert_eq!(cli.kind, DisputeKindArg::PaidVerdictDispute);
        assert_eq!(
            cli.bond_refund_recipient,
            Some(refund_recipient.parse().unwrap())
        );
    }
}
